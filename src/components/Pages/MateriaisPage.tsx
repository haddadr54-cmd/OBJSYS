...
import { FileText, Plus, Edit, Trash2, Search, Filter, Eye, Download, ExternalLink, Video, Link, Image, CheckCircle, AlertCircle, Users } from 'lucide-react';
...
export function MateriaisPage() {
  ...
  const toggleVisualizacoes = async (materialId: string) => {
    if (showVisualizacoes[materialId]) {
      setShowVisualizacoes(prev => ({ ...prev, [materialId]: false }));
    } else {
      try {
        ...
        setShowVisualizacoes(prev => ({ ...prev, [materialId]: true }));
      } catch (error) {
