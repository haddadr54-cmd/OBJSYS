...
import { BookOpen, Calendar, MessageSquare, User, LogOut, GraduationCap, FileText, Star, TrendingUp, AlertTriangle } from 'lucide-react';
          ...
          selectedItemType === 'prova_tarefa' && selectedItem 
            ? (() => {
                ...
                if (isSupabaseConnected) {
                  return turmas.find(t => t.id === prova.disciplina?.turma_id)?.nome;
                } else {
                  const { localDB } = require('../../lib/localDatabase');
                  return localDB.getTurmas().find(t => t.id === (prova as any).turma_id)?.nome;
                }
              })()
            : undefined
        }
          ...
          selectedItemType === 'prova_tarefa' && selectedItem 
            ? (() => {
                ...
                if (isSupabaseConnected) {
                  ...
                  return disciplinas.find(d => d.id === prova.disciplina_id)?.nome;
                }
              })()
            : undefined
        }
          ...
          selectedItemType === 'recado' && selectedItem 
            ? (() => {
                ...
                if (isSupabaseConnected) {
                  ...
                  return usuarios.find(u => u.id === (recado as any).autor_id)?.nome;
                }
              })()
            : undefined
        }
      ...
      {selectedAluno && showStudentProfileModal && (
        <StudentProfileModal
          ...
          onClose={() => {
            ...
            setSelectedAluno(null); // Clear selected student when modal closes
          }}
          aluno={selectedAluno}
          allNotas={allNotas}
