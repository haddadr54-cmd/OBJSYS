...
if (!supabaseUrl || !supabaseAnonKey) {
  console.warn('⚠️ Variáveis de ambiente do Supabase não configuradas. Usando valores padrão.');
}
...
export const getTurmasByProfessor = async (professorId: string): Promise<Turma[]> => {
  try {
    ...
    const turmas = data?.map(d => d.turma).filter(Boolean) || [];
    const turmasUnicas = turmas.filter((turma, index, self) => 
...
export const getRecadosForUser = async (userId: string, userType: string): Promise<Recado[]> => {
  try {
    ...
    if (userType === 'pai') {
      // Pais veem recados gerais, da turma dos filhos e individuais dos filhos
    ...
    // Admins veem todos os recados (sem filtro adicional)
...
export const updateNota = async (id: string, updates: Partial<Nota>): Promise<Nota | null> => {
  try {
    const { data, error } = await supabase
      .from('notas')
      .update(updates)
...
export const deleteNota = async (id: string): Promise<boolean> => {
  try {
    const { error } = await supabase
      .from('notas')
      .delete()
...
// ==================== PERMISSIONS ====================
export const getPermissionCategories = async (): Promise<any[]> => {
  try {
    if (!supabase) return [];
    const { data, error } = await supabase
    ...
    console.error('Erro ao buscar permission categories:', error);
    return [];
  }
};

export const upsertPermissionCategory = async (category: { id: string; name: string; description?: string; icon?: string; color?: string; permissions: string[]; is_custom?: boolean; }): Promise<boolean> => {
  try {
    ...
    console.error('Erro ao upsert permission category:', error);
    return false;
  }
};

export const deletePermissionCategory = async (id: string): Promise<boolean> => {
  try {
    if (!supabase) return false;
    const { error } = await supabase
      .from('permission_categories')
    ...
    console.error('Erro ao deletar permission category:', error);
    return false;
  }
};

export const getUserPermissionOverrides = async (userId: string): Promise<string[]> => {
  try {
    if (!supabase) return [];
    const { data, error } = await supabase
    ...
    console.error('Erro ao buscar user permission overrides:', error);
    return [];
  }
};

export const upsertUserPermissionOverrides = async (userId: string, overrides: string[]): Promise<boolean> => {
  try {
    if (!supabase) return false;
    const { error } = await supabase
      .from('user_permission_overrides')
    ...
    console.error('Erro ao upsert user permission overrides:', error);
    return false;
  }
};
